...
if __name__ == '__main__':
  name='tcw'
  passwd='dhHwn290aewoi'
  time=1
  service='redis'
  email='staugur@qq.com'
  image='redis'
  cid='dshdihairs0'
  ip='127.0.0.1'
  port=6379
  userinfo={"name":name, "passwd":passwd, "time":time, "service":service, "email":email, 'image':image, 'container':cid, 'ip':ip, 'port':port}
  conn=('127.0.0.1', 6379, 1, None)
  c=RedisObject(conn)
  if c.ping():
    print c.keys()
    #print c.hashset(**userinfo)
    print c.hashget(name)
