...
class Docker():
  ...
  def Create(self, **kw):
    ...
    r=self.connect.create_container(image=image, name=name, stdin_open=True, tty=True, ports=cports, host_config=self.connect.create_host_config(restart_policy={"MaximumRetryCount": 0, "Name": "always"}, port_bindings={container_port:host_ip_port}), mem_limit=None, memswap_limit=None, cpu_shares=None)
    cid=r['Id'][:12]
