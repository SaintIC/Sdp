...
userinfo_admin={"name":name, "passwd":passwd, "time":int(time), "service":service, "email":email, 'image':image, 'container':cid, 'ip':Core.Public.SERVER_IP, 'port':int(PORT)}
userinfo_user = r'''
...
''' %(name, name, passwd, int(time), service, email, Core.Public.SERVER_IP, int(PORT))
...
adminconn=('Administrator', 'staugur@vip.qq.com', json.dumps(userinfo_admin))
